...
Whenever a session on rig-4090 touches the project — and always when asked about statuses — sweep the signals and fix any stale rows: `inpr` whose `.status.json` says `done`/`failed` **and** whose artifacts agree ⇒ flip and fill `ended`/`elapsed`; `done` in `.status.json` but expected final artifact missing ⇒ artifacts win, flag it to the user instead of marking done; rows whose evaluations dir vanished ⇒ back to `todo` (outputs were deleted, `started`/`ended`/`elapsed` cleared). Reconciliation is deterministic: md always converges to the signals.
