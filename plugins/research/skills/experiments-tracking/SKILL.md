...
- `wave` — the dispatch this execution belonged to (`YYYYMMDD-HHMMSS`, canon). `gpu` — the GPU set it occupied, opaque identity (a set like `0,1` is legal, but on the shared `behemoth` it is only ever `0` unless the user authorized more for that wave — canon: conventions.md § Rig fleet). Both are read from `.status.json`; before the run starts they come from the generated script's path.
