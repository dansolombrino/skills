...
description: Port source material into Flywheel nodes, artifacts, and durable edges with Flywheel MCP or CLI, without execution or compute spend. Use for source-to-graph authoring, not empirical validation, next-step planning, or autonomous execution.
---
...
If exact tool or field semantics are unclear, load the shared [Flywheel MCP tool map](../flywheel/references/flywheel-mcp-tool-map.md) and [interfaces contract](../flywheel/references/INTERFACES.md) before mutating nodes.

Before any graph write or import, run the mandatory
[Research Root Gate](../flywheel/references/research-root-gate.md). Verify by ancestry
that every explicit, focused, recent, source, parent, and control node belongs to the
resolved workspace root; no contextual node can silently replace the root.

In a Research 2.0 project, scientific mode owns graph authoring. Manual mode previews the import and
waits; auto mode may author only inside the approved source scope and canonical root. This skill
never spends compute, launches execution, or changes engineering state.
...
3. Map each stable page, claim, or concept to a node.
   ...
   - In Research 2.0, create each new source or control node with
...
4. Put a graph-native narrative in the node body/readme field (`content`).
   ...
   - Represent supported hypotheses, insights, outcomes, and similar semantics with
     explicit Markdown labels in `content`; never send removed typed body fields.
...
1. Reuse an existing authoring control node when one already governs the same corpus;
   otherwise, in Research 2.0, branch it from the verified canonical root or governing
   source node. Reserve `commit_new_node` for non-Research use or an explicitly
   approved new root.
