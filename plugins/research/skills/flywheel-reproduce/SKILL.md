...
description: Graphify claim-bearing source material in Flywheel with Flywheel MCP or CLI and validate it empirically within a hard maximum budget. Use for reproduction, replication, or claim validation, not planning-only lookahead, source-to-graph authoring without validation, or autonomous frontier expansion.
---
...
[Research Root Gate](../flywheel/references/research-root-gate.md). Verify by ancestry
that every source, claim, start, parent, and control node belongs to the resolved
workspace root. Explicit or recent graph context never overrides that root.

In a Research 2.0 project, read the execution agreement first. Scientific mode owns the validation
branch and claim disposition; engineering mode owns compute acquisition. Manual mode waits at its
respective decision, while auto mode acts only inside its approved scope/envelope. Product-level
approval prompts, budget expansion, unknown lease cleanup, and destructive graph actions remain
protected.
...
2. Resolve the validation contract.
   ...
   - Run the design gate from the shared [experiment design protocol](../flywheel/references/experiment-design-protocol.md) before any compute request: confirm objective, decision criterion, evidence plan, branch strategy, and budget readiness.
...
4. Map each stable page, claim, or concept to a node.
   ...
   - In Research 2.0, create each new source, claim, validation, or control node with
     `flywheel_branch_node` (MCP) or `flywheel nodes:branch` (CLI) from the canonical
     root or another ancestry-verified parent. Reserve `commit_new_node` for
     non-Research standalone nodes or an explicitly approved new root.
...
For the canonical entity model and public contract terminology behind this controller, load the shared [interfaces contract](../flywheel/references/INTERFACES.md).

Use this storage convention:

1. Reuse an existing control node when one already governs the same source or claim
   family; otherwise, in Research 2.0, branch it from the verified canonical root or
   governing claim. Reserve `commit_new_node` for non-Research use or an explicitly
   approved new root.
...
```md
![Loss curve](https://flywheel-artifact/loss-curve.png)

[Evaluation metrics](https://flywheel-artifact/metrics.json)
```
