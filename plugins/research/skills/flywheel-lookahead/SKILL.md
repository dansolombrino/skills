...
Before any graph write, run the mandatory
[Research Root Gate](../flywheel/references/research-root-gate.md). Prove by ancestry
that every explicit, focused, recent, start, parent, and control node belongs to the
resolved workspace root; none of those discovery sources can override the root.

In a Research 2.0 project, scientific mode owns frontier selection and graph mutation. Manual mode
previews the staged plan and waits; auto mode may stage it only inside the approved scientific scope
and canonical root. This skill never spends compute, launches execution, or changes engineering state.
...
3. Map each stable page, claim, or concept to a node.
   ...
   - In Research 2.0, branch every new control or planning node from the canonical
     root or another ancestry-verified parent. A missing parent does not permit a
     standalone node; `commit_new_node` is reserved for non-Research use or an
...
1. Reuse an existing control node when one already governs the same frontier;
   otherwise, in Research 2.0, create it with `flywheel_branch_node` or
   `flywheel nodes:branch` from the verified root or governing frontier. Reserve
   `commit_new_node` for non-Research use or an explicitly approved new root.
