...
Launch machinery for runs and sweeps. Canon: `../research-project-init/references/conventions.md`. Templates: [references/templates.md](references/templates.md). Dispatch always happens **from rig-4090** (the hub). GitHub distributes one tested, tagged commit to every assigned rig through `$rig-sync`; rsync is only for artifacts. If the Git gate fails, stop instead of copying source ad hoc.
...
4. Run `$rig-sync deploy-revision` first as a dry run, obtain the already-scoped assignment
   ...
   Do not launch any rig unless every rig reports `HEAD == wave tag == approved SHA` and a clean
   execution tree.
...
1. Runs `$rig-sync verify-revision` for its rig immediately before launch, then **dispatches each
   ...
   before Python. Source-drift exit `86` stops the lane; ordinary run failures continue the queue.
...
- **Recover, lane by lane** — first re-run `verify-revision` for the wave's original SHA. If it
...
- Put the exact proposed launch entry in the assignment proposal (`research-journal` skill). On
  ...
  the wave, rationale, rig/GPU assignment, and any one-off behemoth authorization.
