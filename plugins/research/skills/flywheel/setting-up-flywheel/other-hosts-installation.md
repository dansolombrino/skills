...
Alternative: for any host that supports MCP OAuth, use its connector installation flow and set the
MCP URL to `https://flywheel.paradigma.inc/mcp-server`. This may prompt re-authorization more often
than the API-key route depending on the host token lifecycle and refresh behavior.
