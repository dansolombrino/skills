...
- `flywheel_commit_new_node`: publish a caller's locally staged new node; it does not
  require an existing-node stage lease or `base_committed_revision`.
- `flywheel_commit_node`: publish the caller's full staged payload for an existing
  node once terminal and contract-complete; requires an active stage lease and
  explicit `base_committed_revision`.
