...
unless one shared explanation is unambiguous. Ground the proposal in the experiment brief and metric
definition; stop rather than guess.
...
Before execution, load `flywheel-mcp-tool-map.md` from this reference directory and
verify the exact tool surface exposed by the current MCP host before critical flows.
In Research 2.0, also run `research-root-gate.md`: every new design or execution
node must branch from the canonical root or another ancestry-verified parent.
...
1. `mcp__flywheel__flywheel_branch_node` from a verified parent in Research 2.0;
   `mcp__flywheel__flywheel_commit_new_node` only for non-Research standalone work
   or an explicitly approved new root
